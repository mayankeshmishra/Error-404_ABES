...
class OfflineTicket extends StatefulWidget {
  ...
  final driverNumber;
  final driverName;
  OfflineTicket({this.qrData, this.from, this.to, this.busNo, this.noOfTickets,@required this.driverName, this.driverNumber});
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return OfflineTicketState();
  }
}

class OfflineTicketState extends State<OfflineTicket> {
   void initState() {
  super.initState();
      ...
      await for (var snapshot in db.collection("AllUsers").document(
        ...
        for (var i in snapshot.data["allBookings"]) {
          if (i["transactionId"] == transactionId && i["isVerified"] == true) {
            ...
            Navigator.push(context, MaterialPageRoute(builder: (context)=>ShowJourneyNavigationPage(busNo: busNo, from: from, to: to, driverName: widget.driverName, driverNumber: widget.driverNumber,)));
          }
        }
      }
    }
                  ...
                  child: RaisedButton(
                    ...
                    onPressed: () {
                      Navigator.push(
                          ...
                          MaterialPageRoute(
                              builder: (context) =>
                                  ShowNavigationPage(busNo: widget.busNo, from: widget.from, to: widget.to)));
                    },
                  ),
                )
              ],
